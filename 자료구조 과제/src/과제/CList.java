...
public class CList <E>{
	...
	public void insert(E newItem) { // last가 가리키는 노드의 다음에 새노드 삽입
		...
		if(last == null) {						// 리스트가 epmty일때
			newNode.setNext(newNode);
			last = newNode;
		}
		else {
			newNode.setNext(last.getNext());	//newNode의 다음노드가 last가 가리키는 노드의 다음노드가 되도록
			last.setNext(newNode);				//last가 가리키는 노드의 다음노드가 newNode가 되도록
		}
		size++;
	}
	
	public Node delete() { // last가 가리키는 노드의 다음노드를 제거
		if(isEmpty())
		...
		if(x == last) 					//리스트에 1개의 노드만 있는 경우
			...
			last.setNext(x.getNext());	//last가 가리키는 노드의 다음 노드가 x의 다음노드가 되도록 
